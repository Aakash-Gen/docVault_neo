...
function ProfileView() {
  ...
  const fetchFiles = async () => {
    const result = await getAllNFTs(id);
...
export function FileCard({ imageUrl, name }) {
  return (
    <div className="relative bg-gray-800 p-0 rounded-lg text-white">
      <img src={imageUrl} className="rounded-t" />

      <div className="flex justify-between py-1.5 px-2 items-start ">
        <h3 className="text-sm text-gray-300 truncate font-semibold">{name}</h3>
      </div>
    </div>
  );
}
